...
export function useViewModel() {
    ...
    async function enterLottery() {
        ...
        //         objects.value[patternList.value[i] - 1].element.style.backgroundColor = rgba(cardColor.value, Math.random() * 0.5 + 2)
        //     }
        // }
        // 球体形成时，随机设置50%的卡片为小米橙，其余为淡紫色
        const orangeRatio = 0.5 // 橙色卡片占比
        for (let i = 0; i < objects.value.length; i++) {
            const isOrange = Math.random() < orangeRatio
            const colorToUse = isOrange ? '#FF6700' : cardColor.value
            objects.value[i].element.style.backgroundColor = rgba(colorToUse, Math.random() * 0.1 + 0.8)
        }
